...
Here's a KVL mesh analysis example. The base circuit is generated by netlistsvg, then a script parses the SVG and adds node labels, a mesh current ellipse, and voltage polarity marks. In a Reveal.js slide, each annotation layer appears progressively on arrow-key press.

![KVL circuit with all annotation layers visible: node labels, mesh current, voltage polarities](kvl_annotated.png){fig-alt="KVL mesh analysis circuit with node labels (a,b,c,d), clockwise mesh current ellipse, and voltage polarity marks on R1 and R2" width="50%"}
